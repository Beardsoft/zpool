// Copied from https://github.com/karlseguin/zqlite.zig/ and changed to work with Zig 0.13.0
...
pub const Conn = struct {
    ...
    pub fn init(path: [*:0]const u8) !Conn {
        const full_flags = OpenFlags.Create | OpenFlags.EXResCode | OpenFlags.ReadWrite;
...
fn testDB() Conn {
    var conn = open(":memory:") catch unreachable;
    conn.execNoArgs(
        \\
        \\ create table test (
